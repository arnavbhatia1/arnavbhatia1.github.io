...
import { MONO_LINK, RULED } from "../lib/styles";
import type { Project } from "../lib/types";

export const metadata = {
  title: "Projects | Arnav Bhatia",
};
...
const PERSONAL: Project[] = [
  ...
  { title: "FinancialMCP: Published MCP Server", blurb: "Open-source MCP server published to PyPI with 42 financial intelligence tools. Fundamentals, momentum, SEC EDGAR filings and insider trades, FRED macro indicators, Treasury yield curve, CFTC commitment of traders data, Google Trends, market regime detection, VIX analysis, and anomaly scanning. No API keys required for core functionality. Installable via pip, runnable via uvx. Compatible with Claude Desktop, Claude Code, and any MCP-compatible AI agent.", impact: "Published open-source package with real users, real API surface area to maintain, and real consequences for breaking changes. 50 automated tests.", tech: ["Python","MCP","yfinance","SEC EDGAR","FRED","CFTC","Treasury.gov","Google Trends","PyPI"], code: "https://github.com/arnavbhatia1/FinancialMCP", demo: "https://pypi.org/project/financial-mcp-server/" },
    ...
    {projects.map((p) => (
      <article key={p.title} className="py-8">
        <div className="flex flex-wrap items-baseline justify-between gap-4">
          ...
          {(p.demo || p.code) && (
            <div className="flex flex-wrap items-baseline gap-x-6 font-mono text-xs uppercase tracking-[0.12em]">
              {p.demo && (
                <a href={p.demo} target="_blank" rel="noreferrer" className={MONO_LINK}>
                  Demo ↗
                </a>
              )}
              {p.code && (
                <a href={p.code} target="_blank" rel="noreferrer" className={MONO_LINK}>
                  Code ↗
                </a>
              )}
            </div>
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-5 py-12 md:py-16">
      ...
      <Tabs
        ...
        panels={{
          work: <ProjectList projects={WORK} />,
          personal: <ProjectList projects={PERSONAL} />,
        }}
      />
    </main>
  );
